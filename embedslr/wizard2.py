...
The pipeline (embedding → ranking → ABF indicators → ZIP).
"""
...
# ────────── ABF report helper ────────────────────────────────────────────
def _write_abf_report(df_ranked: pd.DataFrame, path: Path, *, top_n: int | None) -> None:
    """
    Saves a minimal bibliometric report with only A, B, F indicators.

    A: indicator_a  – Avg shared references / pair  (float)
    B: indicator_b  – Avg shared keywords  / pair  (float)
    F: indicator_f  – Pairs with ≥1 shared keyword (int)
    """
    # Subset (respect Top‑N if provided)
    sub = df_ranked.head(top_n) if top_n else df_ranked

    # Compute once-prepared stats and pass to individual indicators
    from .bibliometrics import indicator_a, indicator_b, indicator_f, _prepare_stats as _bm_prepare_stats
    stats = _bm_prepare_stats(sub)

    values = {
        "A": indicator_a(sub, _stats=stats),
        "B": indicator_b(sub, _stats=stats),
        "F": indicator_f(sub, _stats=stats),
    }
    ...
    lines = ["==== BIBLIOMETRIC INDICATORS (A, B, F) ===="]
    for k in ("A", "B", "F"):
        v = values[k]
...
def _pipeline(
    ...
    # NOTE: no longer importing full_report; we emit a minimal ABF report.
    ...
    # 6. Minimal bibliometric report (A, B, F only)
    rep = out / "biblio_report.txt"  # keep filename for backward compatibility
    _write_abf_report(ranked, rep, top_n=top_n)
